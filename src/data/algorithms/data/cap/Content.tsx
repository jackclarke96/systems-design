      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        <div className="flex justify-center">
          <img 
            ...
            className="w-40 bg-white rounded-lg p-2"
          />
        </div>
        <p className="text-xs text-center text-muted-foreground">Distributed nodes (n1, n2, n3)</p>
        
        <div className="grid md:grid-cols-3 gap-3">
          <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900/50 rounded-lg p-3">
            <p className="font-bold text-blue-700 dark:text-blue-400 mb-1 text-sm">Consistency</p>
          ...
          <div className="bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-900/50 rounded-lg p-3">
            <p className="font-bold text-green-700 dark:text-green-400 mb-1 text-sm">Availability</p>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Every request gets a response.
            </p>
          </div>

          <div className="bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-900/50 rounded-lg p-3">
            <p className="font-bold text-purple-700 dark:text-purple-400 mb-1 text-sm">Partition Tolerance</p>
            <p className="text-xs text-muted-foreground leading-relaxed">
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <div className="bg-card border border-border rounded-xl p-5 space-y-3">
          ...
          <div className="flex justify-center py-2">
            <img 
              ...
              className="w-44 bg-white rounded-lg p-2"
            />
          </div>
          <p className="text-xs text-center text-muted-foreground">Keys (k) map to servers (s) clockwise</p>
          <div className="text-xs space-y-1 text-muted-foreground border-t border-border pt-3">
            ...
            <p><strong className="text-foreground">Virtual nodes:</strong> Multiple positions per server for better distribution</p>
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-xl p-5 space-y-3">
          ...
          <div className="flex justify-center py-2">
            <img 
              src={capNodes} 
              alt="Replicated nodes" 
              className="w-36 bg-white rounded-lg p-2"
            />
          </div>
          <p className="text-xs text-center text-muted-foreground">Data replicated across nodes</p>
          <div className="text-xs space-y-1 text-muted-foreground border-t border-border pt-3">
            <p><strong className="text-foreground">Strategy:</strong> Map key to ring, store on first <strong>N</strong> clockwise servers</p>
            <p><strong className="text-foreground">Geo-distribution:</strong> Place replicas in <strong>distinct data centers</strong></p>
          </div>
        </div>
      </div>
    </section>
      ...
      <div className="bg-muted/50 rounded-xl p-5 space-y-4">
        ...
        <div className="flex justify-center py-2">
          <img 
            ...
            className="w-72 bg-white rounded-lg p-2"
          />
        </div>
        <p className="text-xs text-center text-muted-foreground">Coordinator sends put() and waits for ACKs</p>
    ...
    <section className="border-t border-border pt-8">
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/50 rounded-xl p-5 space-y-3">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed">
            Use <strong>anti-entropy protocols</strong> to keep replicas synchronized.
          </p>
          <div className="bg-white/50 dark:bg-black/20 rounded-lg p-3 space-y-3">
            <p className="text-xs font-medium text-foreground">Merkle Trees</p>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Hash data in buckets hierarchically. Compare root hashes—if different, traverse down to find which buckets differ.
            </p>
            <img 
              src={capMerkleTree} 
              alt="Merkle trees comparing data buckets" 
              className="w-full rounded-lg"
            />
            <p className="text-xs text-muted-foreground text-center">
              Red = hash differs. Only sync differing keys.
            </p>
          </div>
        </div>
      </div>
